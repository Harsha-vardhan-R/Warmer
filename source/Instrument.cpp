...
void Instrument::midiInAndProcessAudioInplace(juce::MidiBuffer& m, juce::AudioBuffer<float>& a) {
    nodeProcessingQueue.processMidiGiveAudio(m, a);
}

void Instrument::prepareNodeProcessingQueue(double sampleRate, int samplesPerBlock) {
    nodeProcessingQueue.setBufferSizeAndRate(sampleRate, samplesPerBlock);
}
...
bool Instrument::updateTreeParams() {
    ...
    if (deviceManager) {
        ...
        // for the midi message collector.
        // reset(sampleRate);
    } else {
